...
OPENSLIDE_PATH = r'C:/Users/adfec/Downloads/openslide-win64/bin'
...
def main():
    ...
    for epoch in range(args.nepochs):
        ...
        train_loss, train_fnr = train_single(epoch, embedder, rnn, train_loader, criterion, optimizer)
        val_loss, val_fnr = test_single(epoch, embedder, rnn, val_loader, criterion)

        fconv = open(os.path.join(args.output,'convergence.csv'), 'a')
        fconv.write('{},{},{},{},{}\n'.format(epoch+1, train_loss, train_fnr, val_loss, val_fnr))
        fconv.close()
...
def train_single(epoch, embedder, rnn, loader, criterion, optimizer):
    ...
    print('Training - Epoch: [{}/{}]\tLoss: {}\tFNR: {}'.format(epoch+1, args.nepochs, running_loss, running_fns))
    return running_loss, running_fns
...
def test_single(epoch, embedder, rnn, loader, criterion):
    ...
    # running_fps = running_fps/(np.array(loader.dataset.targets)==0).sum()
    running_fns = running_fns/(np.array(loader.dataset.targets)==1).sum()
    print('Validating - Epoch: [{}/{}]\tLoss: {}\tFNR: {}'.format(epoch+1, args.nepochs, running_loss, running_fns))
    return running_loss, running_fns
